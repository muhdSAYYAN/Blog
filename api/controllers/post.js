     ...
     jwt.verify(token, "jwtkey", (err, userInfo) => {
       ...
       db.query(q, [postId, userInfo.id], (err, data) => {
         ...
         if (data.affectedRows === 0) {
           // No rows were affected, meaning the post with given id and uid doesn't exist
           return res.status(404).json("Post not found or user does not have permission");
         }
...
export const updatePost = (req,res)=>{
     ...
     jwt.verify(token, "jwtkey", (err, userInfo) => {
       if (err) return res.status(403).json("Token is not valid!");
        ...
        const postId = req.params.id;
       const q= "UPDATE posts SET `title`=?,`desc`=?,`img`=?,`cat`=? WWHERE `id`=? AND `uid`=? "
